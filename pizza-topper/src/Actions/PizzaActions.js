...
const createPizzaSuccess = (pizza) => ({ type: CREATE_PIZZA_SUCCESS, payload: pizza })
...
const fetchPizzaSuccess = (pizza) => ({ type: FETCH_PIZZA_SUCCESS, payload: pizza })
...
const updatePizzaSuccess = (pizzaId, newPizza) => ({ type: UPDATE_PIZZA_SUCCESS, payload:{pizzaId, newPizza} })
...
const deletePizzaSuccess = (pizza) => ({ type: DELETE_PIZZA_SUCCESS, payload:pizza })
...
export const createPizza = (pizza) => dispatch => {
    ...
    fetch('http://localhost:9000/create/pizza', {
        ...
        .then(pizza => {
            dispatch(
                createPizzaSuccess(pizza)
            )
        })
        .catch(err => {
            dispatch(
                createPizzaError(err)
...
export const deletePizza = (pizza) => dispatch => {
    ...
    fetch(`http://localhost:9000/delete/pizza/${pizza}`, {
        ...
        .then(pizza => {
            dispatch(
                deletePizzaSuccess(pizza)

            )
        })
        .catch(err => {
            dispatch(
                deletePizzaError(err)
